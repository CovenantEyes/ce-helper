...
export async function discoverWorkspace(): Promise<WorkspaceAttributes> {
    ...
    if (!folders || folders.length === 0) {
        return { type: ProjectType.NO_FOLDER };
    }

    if (folders.length > 1) {
        // IMPROVE: If opening multiple workspaces is popular at CE, refactor
        //          this to support it.
        return { type: ProjectType.MULTI_FOLDERS };
    }
    ...
    if (!await fileExists(wscriptPath)) {
         return { type: ProjectType.NON_CE };
    }

    const zarfPath = path.join(basePath, 'zarf');
    if (!await folderExists(zarfPath)) {
        return { type: ProjectType.ZARF_NOT_CLONED };
    }
    ...
    if (!await fileExists(wafPath) || !await fileExists(wafexecPath)) {
        return { type: ProjectType.ZARF_SUBMODULE_NOT_INITIALIZED };
    }
